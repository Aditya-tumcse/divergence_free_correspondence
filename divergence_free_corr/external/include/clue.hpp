...
#define clue_VERSION                                                           \
  clue_STRINGIFY(clue_MAJOR) "." clue_STRINGIFY(                               \
      clue_MINOR) "." clue_STRINGIFY(clue_PATCH)
...
#if !defined(clue_LOG_TO_CONSOLE) && !defined(clue_LOG_TO_FILE) &&             \
    !defined(clue_LOG_TO_STRING) && !defined(clue_LOG_TO_DEBUGGER) &&          \
    !defined(clue_LOG_TO_EVENTLOG) && !defined(clue_LOG_TO_SYSLOG)
#if defined(_WINDOWS)
...
#define clue_LOG_LOGGED_SEVERITIES()                                           \
  clue_LOG_EXPRESSION(clue_LOG_SEV_NONE,                                       \
                      clue::to_severities_text(clue_LOG_LEVEL_BUILD))
...
#define clue_LOG_EMERGENCY(expr)                                               \
  clue_LOG_EXPRESSION(clue_LOG_SEV_EMERGENCY, expr)
#else
  ...
  do {                                                                         \
    if (clue_is_active_build(severity)) {                                      \
      if (clue_is_active(severity)) {                                          \
        std::clog << clue::now_text() << std::setw(clue_LOG_PREFIX_WIDTH)      \
                  << clue::to_severity_text(severity)                          \
                  << clue::to_module_text(clue_LOG_MODULE_NAME) << ": "        \
                  << expr << "\n";                                             \
      }                                                                        \
    }                                                                          \
  } while (clue::is_true(false))
...
#define clue_LOG_EXPRESSION(severity, expr)                                    \
  do {                                                                         \
    if (clue_is_active_build(severity)) {                                      \
      if (clue_is_active(severity)) {                                          \
        clue::filelog() << clue::now_text()                                    \
                        << std::setw(clue_LOG_PREFIX_WIDTH)                    \
                        << clue::to_severity_text(severity)                    \
                        << clue::to_module_text(clue_LOG_MODULE_NAME) << ": "  \
                        << expr << "\n";                                       \
      }                                                                        \
...
#define clue_LOG_EXPRESSION(severity, expr)                                    \
  do {                                                                         \
    if (clue_is_active_build(severity)) {                                      \
      if (clue_is_active(severity)) {                                          \
        clue::windbg() << std::setw(clue_LOG_PREFIX_WIDTH)                     \
                       << clue::to_severity_text(severity)                     \
                       << clue::to_module_text(clue_LOG_MODULE_NAME) << ": "   \
                       << expr;                                                \
      }                                                                        \
...
inline std::string text_or(std::string const &text,
                           std::string const &or_text) {
  return text.length() ? text : or_text;
}

inline std::string text_with_or(std::string const &prefix,
...
inline std::string to_severity_text(int const severity) {
  assert(clue_LOG_SEV_NONE <= severity && severity <= clue_LOG_SEV_MAX &&
         "invalid severity");
  ...
  std::string const cont[] = {
      clue_LOG_SEV_EMERGENCY_TEXT, clue_LOG_SEV_ALERT_TEXT,
      clue_LOG_SEV_CRITICAL_TEXT,  clue_LOG_SEV_ERROR_TEXT,
      clue_LOG_SEV_WARNING_TEXT,   clue_LOG_SEV_NOTICE_TEXT,
      clue_LOG_SEV_INFO_TEXT,      clue_LOG_SEV_DEBUG_TEXT,
  };
  return cont[severity];
}

inline std::string to_severities_text(int const level,
                                      std::string const &postfix = ".",
                                      std::string const &result = "") {
  if (level < 0)
    return result + postfix;

  return to_severities_text(level - 1, postfix,
                            to_severity_text(level) +
                                text_with_or(", ", result, "", ""));
}
...
public:
  ...
  template <typename T> filelog &operator<<(T const &that) {
    stream << that;
    return *this;
  }
...
class strlog {
public:
  strlog() : severity_(clue_LOG_SEV_NONE), stream() {}
  ...
  void severity(int const sev) { severity_ = sev; }

  int severity() const { return severity_; }

  std::string text() const { return stream.str(); }

  template <typename T> strlog &operator<<(T const &that) {
    stream << that;
    return *this;
  }
...
public:
  ...
  ~windbg() { OutputDebugString(stream.str().c_str()); }

  template <typename T> windbg &operator<<(T const &that) {
    stream << that;
    return *this;
  }
...
public:
  ...
  ~evtlog() {
    ...
    const ::HANDLE hlog =
        ::RegisterEventSource(0, text_or(module, "[clue]").c_str());
    ...
    ::ReportEvent(
        ...
        ,
        strings // LPCTSTR *lpStrings,  // array of strings to merge with
                // message
        ,
  ...
  template <typename T> evtlog &operator<<(T const &that) {
    stream << that;
    return *this;
  }
...
class syslog {
public:
  syslog(int const severity) : severity(severity), stream() {}
  ...
  template <typename T> syslog &operator<<(T const &that) {
    stream << that;
    return *this;
  }
