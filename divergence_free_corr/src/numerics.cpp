...
namespace adi{
 namespace numerics{
        const std::vector<adi::deformation_field::BasisIndices> GenerateBasisIndices(const uint32_t &max_number_of_velocity_basis)
        {
            ...
            for(uint32_t i = 1;i <= max_number_of_velocity_basis;++i)
            {
                ...
                for(uint32_t j = 1;j <= max_number_of_velocity_basis;++j)
                {
                    ...
                    for(uint32_t k = 1;k <= max_number_of_velocity_basis;++k)
                    {
                        ...
                        double eigen_val = std::pow((pi_squared * (i_squared + j_squared + k_squared)),-1.5);
            ...
            std::sort(basis_indices.begin(), basis_indices.end(), [](const adi::deformation_field::BasisIndices &a, const adi::deformation_field::BasisIndices &b) {
        ...
        //             adi::deformation_field::DeformationField::computeVelocityFieldTemplated(coeffs_ak, vel_basis_functions);
        ...
        //             adi::deformation_field::DeformationField::computeVelocityFieldTemplated(coeffs_ak, updated_vel_basis_functions);
        ...
        //         // Map coefficients
        //         Eigen::Map<const Eigen::Matrix<T, MAX_NUMBER_OF_VELOCITY_BASIS, 1>> coeffs(coeffs_ak, MAX_NUMBER_OF_VELOCITY_BASIS);

        //         // Map target cloud points to templated type
        //         Eigen::Matrix<T, NUMBER_OF_SAMPLE_POINTS, 3> target_point = s_target_point.template cast<T>();

        //         // Map source points to templated type
        //         Eigen::Matrix<T, NUMBER_OF_SAMPLE_POINTS, 3> src_points = s_src_cloud.template cast<T>();

        //         Fastor::Tensor<T, NUMBER_OF_SAMPLE_POINTS, MAX_NUMBER_OF_VELOCITY_BASIS, TENSOR_DEPTH> templated_vel_basis_functions(s_vel_basis_functions);

        //         auto new_src_cloud = RungeKutta2IntegrationTemplated<T>(s_base_indices, src_points, coeffs, templated_vel_basis_functions, 10);
                
        //         Eigen::Matrix<T, NUMBER_OF_SAMPLE_POINTS, 1> residuals = utilities::L2Norm<T>(new_src_cloud, target_point);
                
        //         // Compute weighted L2 norms
        //         Eigen::Matrix<T, NUMBER_OF_SAMPLE_POINTS, 1> weighted_residuals = residuals * s_weight.template cast<T>().array();
        ...
        //     const Fastor::Tensor<double, NUMBER_OF_SAMPLE_POINTS, MAX_NUMBER_OF_VELOCITY_BASIS, TENSOR_DEPTH> s_vel_basis_functions;
        //     const Eigen::VectorXd s_weight;
        ...
        //         Eigen::MatrixXd broadcasted_target_point = Eigen::MatrixXd::Ones(NUMBER_OF_SAMPLE_POINTS, 3);
        ...
        //             new ceres::AutoDiffCostFunction<MatchingCostFunctor, 1, MAX_NUMBER_OF_VELOCITY_BASIS>(new MatchingCostFunctor(broadcasted_target_point, utilities::toEigenMatrix(src_cloud),vel_basis_functions, weight, base_indices)),
        //             nullptr,
        //             coeffs_ak->data() 
        //         );
        //     }
