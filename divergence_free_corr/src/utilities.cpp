...
double eucledianDistance(const Eigen::Vector3d &point_1,
  ...
  return (std::sqrt(std::pow((point_1[0] - point_2[0]), 2) +
                    std::pow((point_1[1] - point_2[1]), 2) +
                    std::pow((point_1[2] - point_2[2]), 2)));
}

Eigen::MatrixXd
computeDistanceMatrix(const std::vector<Eigen::Vector3d> points) {
  Eigen::MatrixXd distance_matrix =
      Eigen::MatrixXd::Zero(points.size(), points.size());
...
computeCorrespondences(std::vector<adi::Point> source_point_cloud,
  ...
  std::unique_ptr<std::vector<std::pair<adi::Point, adi::Point>>>
      correspondences =
          std::make_unique<std::vector<std::pair<adi::Point, adi::Point>>>();
  ...
  for (adi::Point &source_point : source_point_cloud) {
    ...
    if (kdtree.nearestKSearch(source_descriptor, 1, nn_indices, nn_distances) >
        0) {
      int target_index = nn_indices[0];
      correspondences->push_back(
          std::make_pair(source_point, target_point_cloud[target_index]));
    }
  }
...
Eigen::MatrixXd toEigenMatrix(const std::vector<adi::Point> &cloud) {
  ...
  // Fill the flat_data vector with individual components (x, y, z) of each
  // point
  ...
  Eigen::MatrixXd matrix = Eigen::Map<Eigen::Matrix<double, Eigen::Dynamic, 3>>(
      flat_data.data(), cloud.size(), 3);
...
std::vector<adi::Point> toPointCloud(const Eigen::MatrixXd &cloud) {
  ...
  for (size_t i = 0; i < cloud.rows(); ++i) {
    adi::Point pt;
    if (std::isnan(cloud(i, 0)) || std::isnan(cloud(i, 1)) ||
        std::isnan(cloud(i, 2)) || std::isinf(cloud(i, 0)) ||
        std::isinf(cloud(i, 1)) || std::isinf(cloud(i, 2))) {
      continue;
    }
...
const Eigen::MatrixXd computeLInv(
  ...
  Eigen::MatrixXd L_inv = Eigen::MatrixXd::Zero(MAX_NUMBER_OF_VELOCITY_BASIS,
